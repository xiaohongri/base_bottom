...
import com.example.base_bottom.baidu_dingwei.util.PhotoUtil
import com.example.base_bottom.baidu_dingwei.util.toast
...
class MainActivity_ditu : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_main_ditu)
    ...
    private fun initObserve(){
        vm.busList.observe(this, Observer {
            if(it.status == 200 && it.position != null){
                "园内找到了${it.position!!.size}辆设备".toast()
                for(bus in it.position!!){
                    setBusImg(bus.lng.toDouble(), bus.lat.toDouble())
                }
            }else{
                "没有找到物流设备信息".toast()
            }
    ...
    private fun showDialog(bus: Bus){
        ...
        val date = Date(bus.cur_date)
        val time= simpleDateFormat.format(date)
        val text = "车牌号：${bus.item_id}\n" +
                "时间：$time\n" +
                "地点：${bus.pos}\n"
                //"速度：${bus.speed}\n" +
